# 🕌 Halako Super App for Muslims in Korea

> **A one-stop halal lifestyle platform** connecting Muslims living in and visiting Korea with halal food, prayer spaces, beauty products, and community.

[![Figma Prototype](https://img.shields.io/badge/Figma-Prototype-F24E1E?style=for-the-badge&logo=figma&logoColor=white)](https://www.figma.com/proto/aHfhgBLYAd0Z4qQj6viZNS/Halko?page-id=0%3A1&node-id=13-48&p=f&viewport=429%2C240%2C0.26&t=FAbVp67Ub7Xlwi3h-1&scaling=scale-down&content-scaling=fixed&starting-point-node-id=13%3A48)

---

## 📌 The Problem

Muslims in Korea both residents and tourists face daily friction:

- **"Is this food halal?"** No reliable, centralized verification source
- **"Where can I pray?"** Prayer rooms are scattered and hard to find
- **"Is this cosmetic safe?"** Ingredient labels are in Korean with no halal guidance
- **"Where can I go without discomfort?"** Limited Muslim-friendly infrastructure vs. rapidly growing demand

Korea hosts **98,000 Muslim residents** and receives over **1 million Muslim tourists per year** (33% YoY growth), yet there is no unified platform serving this community.

---

## 💡 The Solution

**Halako** is a super app built specifically for Muslims in Korea. It aggregates halal lifestyle services into a single, trusted, culturally sensitive platform reducing uncertainty, saving time, and building community.

---

## ✨ Key Features

| Feature | Description |
|---|---|
| 🍽️ **Halal Restaurant Finder** | Verified halal dining across Seoul, Busan, and beyond |
| 🕌 **Prayer Room & Mosque Locator** | Real-time map of prayer spaces nationwide |
| 🧴 **AI-Powered Halal Scanner** | OCR + machine learning to verify food and cosmetic ingredients |
| 👗 **Muslimwear & Halal Shops** | Modest fashion and halal beauty product listings |
| 🤝 **Community, Content & Events** | Muslim community hub with travel guides and event listings |

---

## Prototype

Explore the interactive Figma prototype below:

👉 [**View Halako Demo on Figma**](https://www.figma.com/proto/aHfhgBLYAd0Z4qQj6viZNS/Halko?page-id=0%3A1&node-id=13-48&p=f&viewport=429%2C240%2C0.26&t=FAbVp67Ub7Xlwi3h-1&scaling=scale-down&content-scaling=fixed&starting-point-node-id=13%3A48)

---

## 📊 Market Opportunity

- 🌍 **1.3 billion Muslims** worldwide
- 🇰🇷 **$400B+** halal tourism market opportunity in Korea
- 📈 Muslim tourism to Korea growing **33% YoY**
- 84% of surveyed users expressed demand for a Halal Restaurant Finder
- 57.5% requested a Prayer Room & AI Halal Scanner

---

## Business Model

| Revenue Stream | Details |
|---|---|
| **Freemium** | Free base features; paid tier for advanced tools and priority discounts |
| **Marketplace Commissions** | 10% take rate on modest fashion/halal product sales |
| **Business Partnerships** | ~$10/month commission from 1,000+ restaurant and shop listings |
| **Advertising** | Halal brand placements on platform |

**Year 1 Revenue Target: ~$250K USD**  
**Break-even projected: Month 18–20**

---

## Roadmap

| Phase | Timeline | Goal |
|---|---|---|
| **MVP Soft Launch** | 0–3 months | App Store live, Seoul listings, first users onboarded |
| **Growth Optimization** | 3–6 months | Paid ads, referral program, expand nationwide 2,000 MAU |
| **Scale & Expansion** | 6–12 months | 15,000 MAU, 150–180K ARR, 2,000 business partners |
| **Series A Ready** | 12–18 months | 50,000 users, strong monetization, next funding round |

---

## Team

| Name | Role | Responsibility |
|---|---|---|
| Riku Kanamoto | CEO | Vision, direction, partnerships |
| Arij Faiha | CPO | Product design, app structure, feature development |
| Vincen Suvandi | CMO | Branding, marketing strategy, community engagement |
| Alex Rohr | COO | Operations, budgeting, legal/financial planning |

---

## 📁 Project Files

- [`halako-Presentation-2.pdf`](./Halako-Final_organized.pdf) Investor pitch deck
- [`Halako-2.pdf`](./Halako-2.pdf) Business canvas and platform overview

---

## 🏫 About This Project

Halako is a class project developed as part of the **Global MBA Program** at Korea University. It represents an end-to-end startup concept from problem discovery and market research to product design, business modeling, and investor pitch.

---

> *Designed for trust. Built for community.*
